...
//!
//! ```
//! Disclaimer: generate_enum_helpers works with named and unnnamed variant fields. Also it works with variants having
//! no, a single or multiple fields. But it is likely to encounter issues when used with generics and other
...
use syn::{
    ...
    token::{self, And},
    ItemEnum, ItemFn, Lifetime, Token, Type, TypeReference, Variant, Visibility, Field, Fields, FieldsUnnamed, FieldsNamed, TypeTuple, Expr,
};
...
pub fn generate_enum_helper(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    if create_is_functions || create_unwrap_functions || create_unwrap_ref_functions
        || create_unwrap_ref_mut_functions || create_to_tag_functions || create_as_ref_functions
        || create_as_mut_functions || create_get_functions || create_get_ref_functions
...
impl InputEnum {
    ...
    fn variant_type(&self, i: usize) -> Type {
        let elems: Punctuated<_, _> = self.0.variants[i].fields.iter().map(|f| {
            f.ty.clone()
        }).collect();
        ...
        let group = proc_macro2::Group::new(proc_macro2::Delimiter::Parenthesis, proc_macro2::TokenStream::new());
        syn::Type::Tuple(TypeTuple {
            paren_token: token::Paren{ span: group.delim_span() },
            elems
        })
    }
    ...
    fn match_variant_to_tuple(&self, i: usize, enum_ident: Option<Ident>) -> syn::Arm {
        let (pat, body) = match &self.variant(i).fields {
            Fields::Unit => {
                ...
                (self.match_variant(i, None), Box::new(Expr::Tuple(syn::ExprTuple {
                    attrs: vec![],
                    paren_token: token::Paren { span: group.delim_span() },
            ...
            Fields::Named(fields) => {
                ...
                for field in fields.named.iter() {
                    patterns.push(syn::FieldPat {
                        ...
                        colon_token: Some(token::Colon {spans: [Span::call_site(); 1]}),
                ...
                let pat = syn::Pat::Struct(syn::PatStruct {
                    ...
                    qself: None,
                    path: pattern_path,
                    brace_token: token::Brace { span: group.delim_span() },
                    fields: patterns,
                    rest: None,
                });
        ...
        syn::Arm {
            attrs: vec![],
            guard: None,
            fat_arrow_token: token::FatArrow { spans: [Span::call_site(); 2] },
            comma: Some(token::Comma{ spans: [Span::call_site(); 1]}),
            pat,
            body
        }
    }
