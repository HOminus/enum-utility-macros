use crate::{InputEnum, parse_function};
...
use syn::{
    ...
    token::{self, And},
    ItemEnum, ItemFn, Lifetime, Token, Type, TypeReference, Variant, Visibility, Field, Fields, FieldsUnnamed, FieldsNamed, TypeTuple,
};
...
impl<'a> RefEnumBuilder<'a> {
        ...
        for variant in self.input.iter_variants() {
            ...
            fields.iter_mut().for_each(|f| {
                f.ty = Type::Reference(TypeReference {
                    and_token: And { spans: [Span::call_site(); 1] },
    ...
    fn variant_type(&self, i: usize) -> syn::Type {
        let elems: Punctuated<_, _> = self.variants[i].fields.iter().map(|f| {
            f.ty.clone()
        }).collect();
    ...
    pub(crate) fn is_functions(&mut self) {
        ...
        for i in 0..self.input.variant_count() {
            ...
            let pat = self.input.match_variant(i, Some(self.ref_enum_name.clone())); // Same match clause
            let ts = quote! {
                #vs fn #sp (&self) -> bool {
                    matches!(self, #pat)
                }
    ...
    pub(crate) fn to_tag_functions(&mut self) {
        ...
        for i in 0..self.input.variant_count() {
            ...
            arms.push(syn::Arm {
                ...
                pat: self.input.match_variant(i, Some(self.ref_enum_name.clone()))
            });
        }
