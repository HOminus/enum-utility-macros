use crate::{InputEnum, Ident, Span, parse_function};
...
use syn::{
    ...
    token::{self, And},
    ItemEnum, ItemFn, Lifetime, Token, Type, TypeReference, Variant, Visibility, Field, Fields, FieldsUnnamed, FieldsNamed, ExprMatch, Arm, Expr, FieldValue,
};
...
impl<'a> EnumFunctionsBuilder<'a> {
            ...
            let rt = match return_type {
                ty@Type::Path(_) => {
                    Type::Reference(syn::TypeReference{
                        and_token: token::And { spans: [Span::call_site(); 1] },
                        lifetime: None,
                        mutability: None,
                        elem: Box::new(ty)
                    })
                ...
                Type::Tuple(tuple) => {
                    Type::Tuple(syn::TypeTuple {
                        paren_token: tuple.paren_token.clone(),
                        elems: tuple.elems.into_iter().map(|t| {
                            Type::Reference(syn::TypeReference {
                                and_token: token::And { spans: [Span::call_site(); 1] },
                                lifetime: None,
                                mutability: None,
    ...
    pub(crate) fn to_tag_function(&mut self) {
        let vs = self.input.vis();
        let sp = Ident::new(format!("to_tag").as_str(), Span::call_site());

        let tag_ident = Ident::new(format!("{}Tag", self.input.0.ident.to_string()).as_str(), Span::call_site());
    ...
    pub(crate) fn as_ref_functions(&mut self) {
        let vs = self.input.vis();
        let sp = Ident::new(format!("as_ref").as_str(), Span::call_site());

        let ref_ident = Ident::new(format!("{}Ref", self.input.0.ident.to_string()).as_str(), Span::call_site());
    ...
    pub(crate) fn as_mut_functions(&mut self) {
        let vs = self.input.vis();
        let sp = Ident::new(format!("as_mut").as_str(), Span::call_site());

        let ref_ident = Ident::new(format!("{}Mut", self.input.0.ident.to_string()).as_str(), Span::call_site());
